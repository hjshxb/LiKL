...
Taken form https://github.com/cvg/SOLD2
"""
import torch
import numpy as np
...
class additive_gaussian_noise(object):
    ...
    def __call__(self, input_image):
        ...
        noisy_image = (input_image + noise).clip(0., 255.).astype(np.uint8)
...
class random_gray(object):

    def __init__(self, p=None):
        # If the p is not given, use the default setting
        if p is None:
            self.p = 0.5
        else:
            self.p = p
    ...
    def __call__(self, input_image):
        ...
        return np.array(self.transform(input_image), dtype=np.uint8)

class random_colorJitter(object):

    def __init__(self, brightness=0.5, contrast=0.5, saturation=0.2, hue=0.2):
...
class random_saturation(object):
    ...
    def __init__(self, saturation) -> None:
        ...
        # Initialize the transformer
        self.transform = transforms.ColorJitter(saturation=self.saturation)
...
class random_hue(object):
    ...
    def __init__(self, hue) -> None:
        # If the hue is not given, use the default setting
        if hue is None:
            self.hue = 0.2
        else:
            self.hue = hue
        ...
        # Initialize the transformer
        self.transform = transforms.ColorJitter(hue=self.hue)
...
class normalize_image(object):
    """ Image normalization to the range [-1, 1]. """

    def __init__(self):
        self.normalize_value = 127.5

    def __call__(self, input_image):
        return (input_image / self.normalize_value - 1).astype(np.float32)
