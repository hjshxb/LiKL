...
Taken from https://github.com/cvg/SOLD2/blob/main/sold2/
"""
import cv2
...
def plot_junctions(input_image, junctions, junc_size=3, color=None):
    ...
    # A float type image ranging from 0.~255.
        ...
        raise ValueError(
            "Unknown image data type. Expect 0~1 float or 0~255 uint8.")
    ...
    if not len(junctions.shape) == 2:
        raise ValueError("junctions should be 2-dim array.")
    ...
    if junctions.shape[-1] != 2:
        if junctions.shape[0] == 2:
            ...
            raise ValueError("At least one of the two dims should be 2.")
...
def plot_line_segments(input_image, junctions, line_map, junc_size=3,
        ...
        # record the line segment
            ...
            for idx2 in np.where(line_map_tmp[idx, :] == 1)[0]:
                ...
                segments = np.concatenate(
                    (segments, np.array([p1[0], p1[1], p2[0], p2[1]])[None, ...]), axis=0)
    ...
    for idx in range(segments.shape[0]):
        ...
        cv2.line(image, tuple(seg[:2]), tuple(
            seg[2:]), color=color, thickness=line_width)
...
def plot_line_segments_from_segments(input_image, line_segments, junc_size=3,
    ...
    for idx in range(line_segments.shape[0]):
        ...
        # Also draw the junctions
        cv2.circle(image, tuple(np.flip(seg[0, :])), radius=junc_size, color=(
            0, 255., 0), thickness=3)
        cv2.circle(image, tuple(np.flip(seg[1, :])), radius=junc_size, color=(
            0, 255., 0), thickness=3)
...
def plot_images(imgs, titles=None, cmaps='gray', dpi=100, size=6, pad=.5, save_file=None):
...
def plot_keypoints(kpts, colors='lime', ps=4, marker='o' ,save_file=None):
    ...
    for a, k, c in zip(axes, kpts, colors):
        a.scatter(k[:, 0], k[:, 1], c=c, s=ps, marker=marker, linewidths=0)
    if save_file:
        fig = plt.gcf()
        fig.savefig(save_file, bbox_inches='tight', pad_inches=0.0)


def plot_keypoint_matches(kpts0, kpts1, color=None, lw=1.5, ps=4, indices=(0, 1), a=1., save_file=None):
...
def plot_lines(lines, line_colors='orange', point_colors='cyan',
               ps=4, lw=2, indices=(0, 1), save_file=None):
...
def plot_color_line_matches(lines, correct_matches=None,
                            lw=2, indices=(0, 1), return_color=False, save_file=None):
